...
const Main = ({ children }) => {
  ...
  return (
    <main className='flex flex-col lg:flex-row gap-4 w-full h-full p-0 md:p-4 lg:p-16 text-[#f5f5f5]'>
      <aside
        ref={asideRef}
        className={`bg-[#353839] flex flex-col w-full xl:w-[20rem] shrink-0 items-center justify-around p-4 rounded-md border border-black transition-transform duration-300 ${
          isMobile
            ? showAside
              ? 'fixed'
      ...
      <section
        ref={sectionRef}
        className={`bg-[#353839] w-12/12 md:flex-auto flex-1 flex justify-center rounded-md p-2 md:p-8 border-0 md:border border-black overflow-y-scroll ${
          isMobile ? 'pt-[320px]' : ''
        }`}
      >
        {children}
